...
void WowFlutter::process (juce::AudioBuffer<float>& buffer, const Settings& s, float envelope)
{
    ...
    const float baseDelayMs = 8.0f + 2.0f * weird;
    const float effectMix = juce::jlimit (0.0f, 1.0f,
                                          wowAmt * 0.72f + flutterAmt * 0.58f
                                        + driftAmt * 0.50f + damageAmt * 0.32f
                                        + weird * 0.12f);
    ...
    for (int i = 0; i < buffer.getNumSamples(); ++i)
    {
        ...
        for (int ch = 0; ch < channels; ++ch)
        {
            ...
            float modMs = (organicWow * wowAmt * 1.6f
                         + organicFlutter * flutterAmt * 0.24f
                         + drift * driftAmt * 2.5f) * reactBoost;
